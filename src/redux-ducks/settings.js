...
export default function settingsReducers(state: State = {
    schema: null,
    prefs: {},
    ...
    switch (action.type) {
        case FETCH_PREFS_SUCCESS:
            return {
                ...
                prefs: action.preferences.reduce((obj, pref) => ({
                    ...obj,
                    [pref.key]: pref.value
                }), {}),
...
export function fetchSettings({ignoreCache = false}: FetchOptions = {}) {
    return (dispatch: Function, getState: Function) => {
        ...
        return Server.api().getPreferences()
            .then(
...
export function setSetting(key: string, newValue: PrefValue) {
    return (dispatch: Function, getState: Function) => {
        ...
        dispatch({type: SET_PREF_REQUEST, key: key, newVal: newValue})

        return Server.api().setPreference(key, {value: newValue})
            .then(
                () => dispatch({type: SET_PREF_SUCCESS, key: key, newVal: newValue}),
                error =>
                    dispatch({
...
export function fetchSettingsSchema({ignoreCache = false}: FetchOptions = {}) {
    return (dispatch: Function, getState: Function) => {
        if (!ignoreCache && getState().settings.schema)
            return Promise.resolve(dispatch({type: FETCH_SCHEMA_CACHE, schema: getState().settings.schema}))

        dispatch({type: FETCH_SCHEMA_REQUEST})

        return Server.api().getPreferencesSchema()
